...
pub async fn list_notifications(
    ...
    let notifications: Vec<Notification> =
        res.json().await.context("notifications 역직렬화 실패")?;
    Ok(notifications
        .into_iter()
...
pub async fn mark_read(state: &AppState, user_id: Uuid, req: MarkReadRequest) -> Result<()> {
    ...
    let res = state
        ...
        .send()
        .await
        .context("notifications PATCH 요청 실패")?;
