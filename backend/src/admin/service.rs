...
use super::{dto::AdminContentReportResponse, model::AdminContentReport};
...
/// 변환 함수를 따로 둔다.
fn to_response(row: AdminContentReport) -> AdminContentReportResponse {
...
async fn update_content_report_status(
    ...
    let res = state
        ...
        // 업데이트된 row를 응답으로 받기 위해 return=representation 사용
        ...
        .send()
        .await
        .context("관리자 content_reports UPDATE 요청 실패")?;

    if !res.status().is_success() {
        let body = res.text().await.unwrap_or_default();
        return Err(anyhow!("관리자 content_reports UPDATE 실패: {}", body));
    }

    let rows: Vec<AdminContentReport> = res
        .json()
        .await
        .context("관리자 content_reports UPDATE 응답 역직렬화 실패")?;
