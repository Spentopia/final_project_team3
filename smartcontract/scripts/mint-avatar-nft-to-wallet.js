const anchor = require("@coral-xyz/anchor");
const { PublicKey, SystemProgram } = require("@solana/web3.js");
const spl = require("@solana/spl-token");

const TOKEN_METADATA_PROGRAM_ID = new PublicKey(
  "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
);
const TOKEN_PROGRAM_ID = new PublicKey(
  "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
);
const ASSOCIATED_TOKEN_PROGRAM_ID = new PublicKey(
  "ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJe8bYh"
);
const SYSVAR_INSTRUCTIONS_PUBKEY = new PublicKey(
  "Sysvar1nstructions1111111111111111111111111"
);

function deriveAta(owner, mint) {
  return PublicKey.findProgramAddressSync(
    [owner.toBuffer(), TOKEN_PROGRAM_ID.toBuffer(), mint.toBuffer()],
    ASSOCIATED_TOKEN_PROGRAM_ID
  )[0];
}

async function getOrCreateAta(provider, owner, mint) {
  const account = await spl.getOrCreateAssociatedTokenAccount(
    provider.connection,
    provider.wallet.payer,
    mint,
    owner
  );
  console.log("ATA 주소:", account.address.toBase58());
  return account.address;
}

async function main() {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.Spentopia ?? anchor.workspace.spentopia;
  if (!program) {
    throw new Error("Anchor workspace에서 spentopia program을 찾을 수 없습니다.");
  }

  const user = new PublicKey(
    process.env.AVATAR_NFT_RECIPIENT_WALLET ?? provider.wallet.publicKey.toBase58()
  );
  const mintSeed =
    process.env.AVATAR_NFT_MINT_SEED ??
    `dev-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  if (Buffer.byteLength(mintSeed) > 32) {
    throw new Error("AVATAR_NFT_MINT_SEED는 32바이트 이하여야 합니다.");
  }

  const name = process.env.AVATAR_NFT_NAME ?? "Spentopia Dev Avatar";
  const symbol = process.env.AVATAR_NFT_SYMBOL ?? "SPTA";
  const uri = process.env.AVATAR_NFT_URI;
  if (!uri) {
    throw new Error("AVATAR_NFT_URI 환경변수가 필요합니다.");
  }

  const [platformConfig] = PublicKey.findProgramAddressSync(
    [Buffer.from("platform_config")],
    program.programId
  );
  const [sptTokenAuthority] = PublicKey.findProgramAddressSync(
    [Buffer.from("spt_token_authority")],
    program.programId
  );
  const [avatarMint] = PublicKey.findProgramAddressSync(
    [Buffer.from("avatar_mint"), user.toBuffer(), Buffer.from(mintSeed)],
    program.programId
  );
  const [collectionMint] = PublicKey.findProgramAddressSync(
    [Buffer.from("avatar_collection_mint")],
    program.programId
  );
  const [metadata] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("metadata"),
      TOKEN_METADATA_PROGRAM_ID.toBuffer(),
      avatarMint.toBuffer(),
    ],
    TOKEN_METADATA_PROGRAM_ID
  );
  const [masterEdition] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("metadata"),
      TOKEN_METADATA_PROGRAM_ID.toBuffer(),
      avatarMint.toBuffer(),
      Buffer.from("edition"),
    ],
    TOKEN_METADATA_PROGRAM_ID
  );
  const [collectionMetadata] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("metadata"),
      TOKEN_METADATA_PROGRAM_ID.toBuffer(),
      collectionMint.toBuffer(),
    ],
    TOKEN_METADATA_PROGRAM_ID
  );
  const [collectionMasterEdition] = PublicKey.findProgramAddressSync(
    [
      Buffer.from("metadata"),
      TOKEN_METADATA_PROGRAM_ID.toBuffer(),
      collectionMint.toBuffer(),
      Buffer.from("edition"),
    ],
    TOKEN_METADATA_PROGRAM_ID
  );
  console.log("programId:", program.programId.toBase58());
  console.log("admin:", provider.wallet.publicKey.toBase58());
  console.log("recipient:", user.toBase58());
  console.log("mintSeed:", mintSeed);
  console.log("avatarMint:", avatarMint.toBase58());

  const userTokenAccount = await getOrCreateAta(provider, user, avatarMint);

  const tx = await program.methods
    .mintAvatarNft(mintSeed, name, symbol, uri)
    .accounts({
      platformConfig,
      admin: provider.wallet.publicKey,
      user,
      avatarMint,
      metadata,
      masterEdition,
      sptTokenAuthority,
      userTokenAccount,
      collectionMint,
      collectionMetadata,
      collectionMasterEdition,
      metadataProgram: TOKEN_METADATA_PROGRAM_ID,
      sysvarInstructions: SYSVAR_INSTRUCTIONS_PUBKEY,
      tokenProgram: TOKEN_PROGRAM_ID,
      associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID,
      systemProgram: SystemProgram.programId,
    })
    .rpc();

  console.log("mint_avatar_nft tx:", tx);
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
