...
// 인증 자체는 체인에 직접 쓰는 것이 아니라, 프론트가 nonce를 서명해서 백엔드로 보내고
...
// 백엔드가 내려준 nonce 원문 그대로 서명해야 하므로, UTF-8 인코딩만 담당한다.
export function encodeUtf8(value: string): Uint8Array {
    return new TextEncoder().encode(value);
}

// 서버가 준 nonce를 signMessage로 서명하고 Base58 문자열로 바꿔준다.
...
export async function signNonceToBase58(
    nonce: string,
    signMessage: (message: Uint8Array) => Promise<Uint8Array>,
): Promise<string> {
    const signedBytes = await signMessage(encodeUtf8(nonce));
